...
export default function BookTurf() {
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      const stkResponse = await fetch("/api/payments/mpesa/stkpush", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          endTime: `${String(Number(selectedSlotData!.time.split(":")[0]) + 1).padStart(2, "0")}:${selectedSlotData!.time.split(":")[1]}`,
          guests,
      ...
      const newBookingId = stkResult.bookingId;
                ...
                >
                  ...
                  <div className="grid gap-8 p-6 lg:grid-cols-[1.5fr_1fr]">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        <label className="mb-2 block text-sm font-semibold text-foreground">Guests</label>
                        <input
                          type="number"
                          min={1}
                          max={10}
                          ...
                          className="w-full rounded-lg border-2 border-border bg-background px-4 py-3 text-sm text-foreground outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20"
                        />
                      </div>
                      ...
                      <Button
                        ...
                        size="lg"
                        onClick={handleBook}
                        disabled={!name || !email || !mpesaPhone || isSubmitting}
                        className="w-full"
                      >
                        {isSubmitting ? (
                          <>
              ...
              {paymentState === "success" && (
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground mb-8">
                    Confirmation details have been sent to <span className="font-semibold">{email}</span>
                  </p>
                  <Button
                    variant="hero"
                    size="lg"
